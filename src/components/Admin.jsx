...
const Admin = () => {
  ...
  const { isLoading, admins, error } = useSelector((state) => state.admins);
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1 select-none">
                    <Icon />
                    <span
                      className={`${name === "Remove" ? "text-red-800" : ""}`}
                    >
                      {name}
                    </span>
                  </div>
      ...
      handleClicks: [
        {
          ...
          click: (row) => {
            ...
            const itemId = data.find((item) => item.id === row.id)
              ? data.find((item) => item.id === row.id).id
              : null;
            setEditItem(itemId);
        ...
        },
        {
          ...
          click: (row) => {
            ...
            // console.log(row.id);
            axios
              .delete(`http://localhost:4000/api/user/${row.id}`, {
                withCredentials: true,
              })
              .then((response) => {
                if (response && response.data) {
                  // console.log('deleted user ',response.data)
                  setRawData(rawData.filter((item) => item.id !== row.id));
    ...
    <div className="p-5 flex flex-col gap-5 ">
      <div className="w-full bg-[#141f2f] p-3 flex justify-end rounded-lg border-1 border-white">
        <div className=" ">
          {!checkedData.length ? (
            <div
              className="flex justify-center items-center gap-2 w-[182px] h-9 bg-blue-500 rounded-md text-white px-4 py-0 cursor-pointer select-none"
            ...
            <div className="flex items-center text-lg text-white">
              ...
              <div
                className="flex justify-center items-center gap-2 w-[182px] h-9 bg-[#de353c] rounded-md text-white px-4 py-0 cursor-pointer select-none"
